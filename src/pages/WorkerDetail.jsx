...
import { Tabs, Tab, Box, Table, TableBody, TableCell, TableContainer, TableHead,
         TableRow, Paper, Button, TableSortLabel, TextField,Card, CardContent, Typography, Grid, Switch, FormControlLabel } from '@mui/material';
import workerConfig from '../../src/worker_parameters_config.json';
...
const WorkerDetail = () => {
  ...
  // Get worker details from config (support lookup by IP or name)
  let workerDetail = workerConfig.workers_param_dict[workerId];
  if (!workerDetail) {
    // If not found by key, try to find by worker name
    workerDetail = Object.values(workerConfig.workers_param_dict)
                         .find(w => w.name === workerId);
  }
  if (!workerDetail) {
    // If still not found, display an error
    return (
      <Box sx={{ p: 2 }}>
  ...
  const scenarioData = workerDetail[datasetKey];
  ...
  if (workerDetail.name === workerId) {
    const entry = Object.entries(workerConfig.workers_param_dict)
  ...
  const workerDataRows = [{
    ...
    time_processing_diff: activity.time_processing_diff,
    time_sent: activity.time_sent,
    time_received: activity.time_received,
    time_update_diff: activity.time_update_diff,
    time_decision: activity.time_decision,
    time_exec: activity.time_exec,
    time_allocation_diff: activity.time_allocation_diff,
    actual_label: activity.actual ? activity.actual.label : undefined,
    actual_time_since_start: activity.actual ? activity.actual.time_since_start : undefined,
    predicted_label: activity.predicted ? activity.predicted.label : undefined,
    predicted_label_distribution: activity.predicted ? activity.predicted.label_distribution : undefined,
    predicted_times: activity.predicted ? activity.predicted.times : undefined,
    allocated_task_type: activity.allocated ? activity.allocated.task_type : undefined,
    allocated_tasks: activity.allocated ? activity.allocated.tasks : undefined,
    QoS: activity.QoS,
    estimated_QoS: activity.estimated_QoS,
    lower_bound_QoS: activity.lower_bound_QoS,
    upper_bound_QoS: activity.upper_bound_QoS,
    time_allocation_diff_avg: activity.time_allocation_diff_avg
  }];
